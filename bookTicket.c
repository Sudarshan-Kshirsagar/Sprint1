...
void *updateBusBookingData(){
    // Opening Files
    FILE *fpA = fopen("ahmedabadBus.txt", "w");
    FILE *fpM = fopen("mumbaiBus.txt", "w");
    for(int i=0; i<10; i++)
    {
        fputs(mumbaiToAhmedabadBus[i], fpM); // mumbaiToAhmedabadBus
        fputs(ahmedabadToMumbaiBus[i], fpA); // ahmedabadToMumbaiBus
        fputc('\n', fpM); // mumbaiToAhmedabadBus
        fputc('\n', fpA); // mumbaiToAhmedabadBus
    }
    fclose(fpA);
    fclose(fpM);
} // Complete
    ...
    while(row < bookingDataLength)
    {
        if(strcmp(bookingData[row].userId, userId) == 0)
        {
            ...
            while(tok)
            {
                if(tok[0] == 'M')
                {
                    fromMumbai[fromMumbaiIndex] = tok[0];   ++fromMumbaiIndex;
                    fromMumbai[fromMumbaiIndex] = tok[1];   ++fromMumbaiIndex;
                    ...
                    fromMumbai[fromMumbaiIndex] = tok[6];   ++fromMumbaiIndex;
                    fromMumbai[fromMumbaiIndex] = tok[7];   ++fromMumbaiIndex;
                    fromMumbai[fromMumbaiIndex] = ',';      ++fromMumbaiIndex;
                }
                else if(tok[0] == 'A')
                {
                    fromAhmedabad[fromAhmedabadIndex] = tok[0];   ++fromAhmedabadIndex;
                    fromAhmedabad[fromAhmedabadIndex] = tok[1];   ++fromAhmedabadIndex;
                    ...
                    fromAhmedabad[fromAhmedabadIndex] = tok[6];   ++fromAhmedabadIndex;
                    fromAhmedabad[fromAhmedabadIndex] = tok[7];   ++fromAhmedabadIndex;
...
void printTicket(char sp, char ep, char ticket[], int busFare, int ch){
...
int bookTicket(char se[], char *userId, char startPoint, char endPoint, int ch){
    ...
    char newSeatNo[10] = "";
    newSeatNo[0] = startPoint;
    newSeatNo[1] = endPoint;
    strcat(newSeatNo, se);
    printTicket(sp, ep, s, busFare, choice);
    addSeatIntoBookingData(newSeatNo, userId, ch);
...
void addSeatIntoBookingData(char seatNo[], char *userId, int ch){
    printf("New seatNumber : %s", seatNo);
    int index = -1;
    for(int i=0; i < bookingDataLength; i++)
    {
    ...
    pthread_create(&tempThread, NULL, updateBookingdata, NULL);
...
void cancelBooking(char *userId){
    displayMyBooking(userId);
    char seatNo[10];
    printf("\n\tEnter Seat To Cancel : ");
    scanf("%s", seatNo);
    char userBooking[200] = "";
    int index = -1;
    for(int i=0; i < bookingDataLength; i++)
    {
        if(strcmp(bookingData[i].userId, userId) == 0)
        {
            index = i;
            strcpy(userBooking, bookingData[i].seatNumbers);
            break;
        }
    }
    char newstr[200] = "";
    
    for(int i=0; i < strlen(userBooking); i++)
    {
        if(seatNo[0] == userBooking[i] && seatNo[1] == userBooking[i+1] &&seatNo[2] == userBooking[i+2] && seatNo[3] == userBooking[i+3] && seatNo[5] == userBooking[i+5] && seatNo[7] == userBooking[i+7])
        {
            int kk = 0;
            for(int k = 0; k < strlen(userBooking); k++)
            {
                if(k == i)
                {
                    k = k + 8;
    ...
    printf("\n\tTotal Seat : [%s], Request For : [%s]\n", userBooking, seatNo);
    printf("\n\tRemaining seats : %s\n", newstr);
    strcpy(bookingData[index].seatNumbers, newstr);
    
    pthread_t tempThread;
    pthread_create(&tempThread, NULL, updateBookingdata, NULL); //updateBusBookingData
    
    
    char line[100];
    int row = seatNo[7] - 48; // MAABS0R9
    int col = seatNo[5] - 48; // 01234567
    
    if(seatNo[0] == 'M') // Mumbai to Ahmedabad
        strcpy(line, mumbaiToAhmedabadBus[row]);
    else
        strcpy(line, ahmedabadToMumbaiBus[row]);
    int delemeter = 0;
    ...
    while(tok)
    {
        ...
        {
            ++delemeter;
            int j = (int) strlen(tok);
            int pk = (int) strlen(dummyLine);
            if( j == 2)
            {
                if(seatNo[2] == tok[0] && seatNo[3] == tok[1])
                {
                    dummyLine[strlen(dummyLine)] = 'V';
                }
                else
                    printf("\nData Not Found\n");
            }
            ...
            {    
                int kk = (int) strlen(dummyLine);
                for(int i=0; i < j; i++, kk++)
                {
                    if(seatNo[2] == tok[i] && seatNo[3] == tok[i+1])
                    {
                        i = i + 2;
                    }
                    dummyLine[kk] = tok[i];
            ...
            if(dummyLine[pk] == ',')
            {
                for(int i=0; i < strlen(dummyLine); i++)
                {
                    dummyLine[i] = dummyLine[i+1];
                }
            }
            ++delemeter;
            strcat(dummyLine, "|");
    ...
    pthread_t tempThread1;
    pthread_create(&tempThread1, NULL, updateBusBookingData, NULL); //updateBusBookingData
...
int showRegistrationPage(char *userId){
    ...
    while(whileIsTrue)
    {
        ...
        else if(whileIsTrue == 7) // Save And EXIT
        {
            return 1;
        }
    }
...
int validate(char a){
    ...
    {
        printf("\n\tPlease choose your valid Start Point from 'A' to 'I'!\n");
        return 1;
    }
}  // Complete
...
int validatechar(char a,char b){
    ...
    {
        printf("\n\tPlease choose your valid Stop Point from '%c' to 'I'!\n", a+1);
        return 1;
    }
}  // Complete
